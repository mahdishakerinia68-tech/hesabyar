...
function allSyncRecords(){
  const ks=["accounts","transactions","people","reminders","checks","expenseCats","incomeCats"];
  const out=[];
...
function recordsFromLocal(){
  const ks=["accounts","transactions","people","reminders","checks","expenseCats","incomeCats"],out=[];
  for(const k of ks) for(const r of (data[k]||[])) out.push({id:recordDocId(k,r.id),type:k,record:r,updatedAt:r.updatedAt||new Date().toISOString(),deleted:false});
  for(const k of ks) for(const [id,dt] of Object.entries(data._sync?.tombstones?.[k]||{})) out.push({id:recordDocId(k,id),type:k,record:{id},updatedAt:dt,deleted:true});
  return out;
}
...
async function syncSave(){
  ...
  while(sync.queued){sync.queued=false;try{await pushRest();setSyncStatus("☁️ ذخیره ابری انجام شد — "+dataSummary(data))}catch(e){console.error(e);setSyncStatus("⚠️ ذخیره ابری انجام نشد: "+(e.code||"")+" "+e.message)}}
  sync.saving=false;
}
...
function addCatPrompt(type){const n=prompt(type==="expense"?"نام دسته هزینه:":"نام نوع دریافت:");if(!n?.trim())return;const arr=type==="expense"?data.expenseCats:data.incomeCats;const nc=touch({id:uid(),name:n.trim()});arr.push(nc);markDirty(type==="expense"?"expenseCats":"incomeCats",nc.id,false,nc,nc.updatedAt);save();openCategory()}
...
function savePerson(id){if(!$("pn").value.trim()||!parseMoney($("pa").value))return alert("نام و مبلغ را وارد کنید");const o={type:$("pt").value,name:$("pn").value.trim(),amount:parseMoney($("pa").value),due:$("pd").value,note:$("pnote").value};if(id){const p=data.people.find(x=>x.id===id);Object.assign(p,o);touch(p);markDirty("people",p.id,false,p,p.updatedAt)}else{const np=touch({id:uid(),paid:0,...o});data.people.push(np);markDirty("people",np.id,false,np,np.updatedAt)}save();closeModal()}
function deletePerson(id){if(confirm("این مورد حذف شود؟"))removeRecord("people",id)}
function payPerson(id){const p=data.people.find(x=>x.id===id);if(!p)return;const v=prompt("مبلغ تسویه:",String(p.amount-p.paid));if(v!==null){p.paid=Math.min(p.amount,p.paid+parseMoney(v));touch(p);markDirty("people",p.id,false,p,p.updatedAt);save()}}
