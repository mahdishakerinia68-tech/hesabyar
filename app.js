...
function render(){
 ...
 if($("txList")&&pageActive("transactions"))$("txList").innerHTML=data.transactions.filter(t=>(!q||String(t.title).includes(q)||String(t.category||"").includes(q))&&(!ft||t.type===ft)&&(!fc||t.category===fc||String(t.category||"").startsWith(fc+" - "))).map(safeTxHTML).join("")||empty("تراکنشی پیدا نشد");
