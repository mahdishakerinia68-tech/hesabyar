...
const blankData=()=>({accounts:[],transactions:[],people:[],reminders:[],notes:[],checks:[],invoices:[],customers:[],products:[],audit:[],expenseCats:defaultsExpense.map((name,i)=>({id:"e"+i,name})),incomeCats:defaultsIncome.map((name,i)=>({id:"i"+i,name})),pin:""});
...
data.transactions??=[];data.people??=[];data.customers??=[];data.products??=[];data.reminders??=[];data.notes??=[];data.checks??=[];data.invoices??=[];data.audit??=[];data.expenseCats??=defaultsExpense.map((name,i)=>({id:"e"+i,name}));data.incomeCats??=defaultsIncome.map((name,i)=>({id:"i"+i,name}));data.pin=typeof data.pin==="string"?data.pin:"";data._sync??={tombstones:{}};data._sync.tombstones??={};for(const k of ["accounts","transactions","people","customers","products","reminders","notes","checks","invoices","expenseCats","incomeCats"]){for(const r of data[k]){r.id??=uid();r.updatedAt??=new Date().toISOString()}}
...
function allSyncRecords(){
  const ks=["accounts","transactions","people","reminders","notes","checks","invoices","expenseCats","incomeCats","audit"];
  const out=[];
...
function recordsFromLocal(){
  const ks=["accounts","transactions","people","reminders","notes","checks","invoices","expenseCats","incomeCats","audit"],out=[];
  for(const k of ks) for(const r of (data[k]||[])) out.push({id:recordDocId(k,r.id),type:k,record:r,updatedAt:r.updatedAt||new Date().toISOString(),deleted:false});
  for(const k of ks) for(const [id,dt] of Object.entries(data._sync?.tombstones?.[k]||{})) out.push({id:recordDocId(k,id),type:k,record:{id},updatedAt:dt,deleted:true});
  return out;
}
...
function renderProducts(){const box=$("productList");if(!box)return;box.innerHTML=data.products.map(p=>`<div class="item"><div><b>📦 ${esc(p.name)}</b><div class="meta">${p.code?"کد: "+esc(p.code)+" • ":""}قیمت: ${money(p.price)}</div><div class="meta">موجودی: ${fa(p.stock)} ${Number(p.stock)<=Number(p.minStock)&&Number(p.minStock)>0?" • ⚠️ موجودی کم":""}</div></div><div class="actions"><button onclick="openProduct('${p.id}')">✏️</button><button onclick="deleteProduct('${p.id}')" class="danger-icon">🗑</button></div></div>`).join("")||empty("هنوز کالایی ثبت نشده است")}
...
async function saveNote(id){
 ...
 const o={title,date:jalaliDateTimeToISO($("ndate").value),repeat:$("nrepeat").value,text:$("ntext").value.trim(),items};
...
function invoiceTotal(inv){const sub=invoiceSubtotal(inv),discount=Math.min(sub,Number(inv.discount)||0),tax=Math.max(0,Math.round((sub-discount)*(Number(inv.taxRate)||0)/100));return Math.max(0,sub-discount+tax)}
...
function updateInvoiceLiveTotal(){
 ...
 const dis=Math.min(sub,Number($("invDiscount")?.value)||0),tax=Math.max(0,Math.round((sub-dis)*(Number($("invTax")?.value)||0)/100)),total=Math.max(0,sub-dis+tax);
...
function saveInvoice(id){
 ...
 const discount=Math.max(0,Number($("invDiscount").value)||0),taxRate=Math.max(0,Number($("invTax").value)||0);let paid=Math.max(0,Number($("invPaid").value)||0);
 const o={name,seller,date,number,items,customerId:$("invCustomer").value||"",address:$("invAddress").value.trim(),discount,taxRate,paid,status:$("invStatus").value,total:0};o.total=invoiceTotal(o);if(o.status==="paid")o.paid=o.total;if(o.paid>=o.total&&o.total>0)o.status="paid";else if(o.paid>0)o.status="partial";else o.status="unpaid";
